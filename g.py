import math
print('Simplificador_de_equacoes(0)   faz_sincrono(1)')
opcao = int(input())
if opcao == 0:
    ...
    def Simplificador_de_equacoes_2(v):
        print('Simplifique sua equação booleana')
        x = v
    ...
    def faz_sincrono(v):
        ...
        for i in v:                 #ADICIONA OS BINARIOS BRUTOS DOS NUMEROS REBIDOS
            binarios.append(bin(int(i))[2:])
        for i in binarios:
            print(i)
        ...
        for i in range(len(binarios)):
            if len(str(binarios[i])) > maior_len:
                maior_len = len(binarios[i])

        tabela_binarios = []
        Qtd_de_flip_flops = maior_len

        for i in v:
            tabela_binarios.append(faz_bin(i, maior_len))
        ...
        for i in range(Qtd_de_flip_flops):
            ...
            for b in range(len(v)):
                print(tabela_binarios[b], tabela_binarios[b][Qtd_de_flip_flops - i - 1])
                variaveis.append(tabela_binarios[b][Qtd_de_flip_flops - i - 1])
            print('\n')
            J = ''
            K = ''

            #for p in variaveis:  #PRINTA A COLUNA
                #print(p)

            for z in range(len(variaveis)):
                if z < len(variaveis) - 1:
                    J += tabelinha_do_JK(variaveis[z], variaveis[z + 1])[0]

                    K += tabelinha_do_JK(variaveis[z], variaveis[z + 1])[1]
                else:
                    J += tabelinha_do_JK(variaveis[z], variaveis[0])[0]

                    K += tabelinha_do_JK(variaveis[z], variaveis[0])[1]

            print('------------')

            print('|' + "J"+ str(i), ' |  ', "K" + str(i) + '|')
            ...
            para_tabela = ''
            len_tabela_verdade = 0
            dois = 1
            while len_tabela_verdade < len(v):
                len_tabela_verdade = len_tabela_verdade ** dois
                dois += 1
                print(len_tabela_verdade)
            print(len_tabela_verdade)
